...
async def doDeleteLastCMDMessage(bot, thisChannelID):
    ...
    try:
        lastMessage = await getMessage(
            bot, thisChannelID, dData.getMessID(thisChannelID)
        )
        await lastMessage.delete()
    except:
        pass
    ...
    if dyna[0] == "Day":
        ...
        for d in datas:
            thisMenuEmbed.add_field(
                name=f"[{dUse.fromTerzTimeToStr(hashTime.hashBack(d[0]))}] {d[1][0]}",
                value=str(d[1][1]),
                inline=False,
            )
...
async def menuCmdCommand(chan):
    ...
    if dayOfWeek == -1:
        ...
        thisMenuEmbed.add_field(
            name="📚 จำนวนวิชาที่อยู่ในระบบ",
            value=f"ทั้งหมด {sData.getNSubject(thisChanId)} วิชา",
        )
        thisMenuEmbed.add_field(
            name="📕 จำนวนวิชาในวันนี้",
            value=f"มี {sData.getNSubjectOfDay(thisChanId,dayOfWeek)} วิชา",
        )
        thisMenuEmbed.add_field(
            name="💬 โหมดการแจ้งเตือน",
            value=dData.isNotiDay(thisChanId) and "เตือนทุก ๆ วัน" or "เตือนทุก ๆ วิชา",
        )
        thisMenuEmbed.add_field(
            name="🏖 หยุดเรียนหรือไม่",
            value=dData.getVacation(thisChanId) == 0
            and "เรียนตามปกติ"
            or f"วู้ว หยุดอีก {dData.getVacation(thisChanId)} วัน",
        )
...
async def callFlow(idFlow, bot, thisChannelID, dlcc=None):
    ...
    if idFlow == "callSchedule":
        ...
        if dData.isExistID(thisChannelID):
            ...
            thisEmbed = Embed(
                title="สิ่งที่จะหายไป", description="ห า ย ไ ป", colour=Color.dark_red()
            )
            thisEmbed.add_field(
                name="ข้อมูลตาราง",
                value="ตารางจะหายไปทั้งหมด และหากสร้างใหม่ ข้อมูลเก่าจะไม่กู้คืน(กรอกใหม่)",
                inline=False,
            )
            thisEmbed.add_field(
                name="การตั้งค่า", value="การตั้งค่าจะหายไป", inline=False
            )
            m = await thisChannel.send(
                "**💥💥คุณกำลังจะลบแชลเนลนี้💥💥**\nหากลบแล้ว จะไม่สามารถกู้คืนได้ แน่ใจแล้วหรือไม่",
                embed=thisEmbed,
                components=[
                    ActionRow(
                        Button(
    ...
    # ? ADDD
        ...
        await thisChannel.send(
            ":speech_balloon:กรุณาใส่ชื่อวิชาที่จะต้องการเพิ่ม (ไม่เกิน 20 ตัวอักษร)\n*ส่งมาในแชทนี้เลย*"
        )
        ...
        newTemp = dData.getTemp(thisChannelID)
        pKey = dData.makeNewKey(thisChannelID) + ":"
        thisEm = dUse.getEmbedAllTimeFromSubject(thisChannelID, newTemp[0], newTemp[1])
        m = await thisChannel.send(
        ...
        textTime = hashTime.hashBack(newHash)[0] + " " + hashTime.hashBack(newHash)[1]
        ...
        m = await thisChannel.send(
            f"เวลาถูกไหม????",
            embed=Embed(title=curTemp[0], description=textTime, colour=Color.random()),
            components=[menus],
        )
        dData.addMessageId(thisChannelID, m.id)
    ...
    # ? Edit Goes hereeeee
        ...
        m = await thisChannel.send(
            ...
            components=[
                dUse.makeSelectSubject(thisChannelID, pKey + "edi_SelectSubject"),
                dUse.backToMenu(pKey),
            ],
        )
        dData.addMessageId(thisChannelID, m.id)
        ...
        await thisChannel.send(
            f":speech_balloon:กรุณาใส่ชื่อวิชาใหม่ จากวิชา {dData.getTempInd(thisChannelID, 0)} (ไม่เกิน 20 ตัวอักษร)"
        )
        ...
        for tim in allTimes:
            ...
            thisOptions.append(
                SelectOption(label=f"{dUse.fromTerzTimeToStr(timeText)}", value=tim)
            )

        m = await thisChannel.send(
            f"**กรุณาเลือกเวลาที่ต้องการแก้ไข...**",
            embed=dUse.getEmbedAllTimeFromSubject(thisChannelID, curSubject),
            components=[
                ActionRow(
                    dUse.anyButton(
                        pKey + "edit_chaTimSub", "ย้อนกลับไปที่เมนูแก้ไข", "↩"
                    ),
                ),
                Select(
        ...
        pKey = dData.makeNewKey(thisChannelID) + ":"
        dayInThai = ["อาทิตย์", "จันทร์", "อังคาร", "พุธ", "พฤหัสบดี", "ศุกร์", "เสาร์"]
        fromDay = hashTime.hashBack(dData.getTempInd(thisChannelID, 2))[0]
        SubjectTitle = dData.getTempInd(thisChannelID, 0)
        m = await thisChannel.send(
            ...
            components=[
                ActionRow(
                    dUse.anyButton(
                        pKey + "edi_chaTimeDay_0", "วัน" + dayInThai[0], "🕒"
                    ),
                    dUse.anyButton(
                        pKey + "edi_chaTimeDay_1", "วัน" + dayInThai[1], "🕒"
                    ),
                    dUse.anyButton(
                        pKey + "edi_chaTimeDay_2", "วัน" + dayInThai[2], "🕒"
                    ),
                ),
                ActionRow(
                    dUse.anyButton(
                        pKey + "edi_chaTimeDay_3", "วัน" + dayInThai[3], "🕒"
                    ),
                    dUse.anyButton(
                        pKey + "edi_chaTimeDay_4", "วัน" + dayInThai[4], "🕒"
                    ),
                    dUse.anyButton(
                        pKey + "edi_chaTimeDay_5", "วัน" + dayInThai[5], "🕒"
                    ),
                    dUse.anyButton(
                        pKey + "edi_chaTimeDay_6", "วัน" + dayInThai[6], "🕒"
                    ),
                ),
            ],
        )
        ...
        m = await thisChannel.send(
            ...
            components=[
                ActionRow(
                    ...
                    dUse.anyButton(
                        pKey + "rem_allSubCon_cancel", "ยังก่อน ยังไม่ลบ", "↩"
                    ),
                )
            ],
        )
        ...
        m = await thisChannel.send(
            ...
            components=[
                ActionRow(
                    ...
                    dUse.anyButton(pKey + "rem_subCon_cancel", "ยังก่อน ยังไม่ลบ", "↩"),
                )
            ],
        )
        dData.addMessageId(thisChannelID, m.id)
        ...
        m = await thisChannel.send(
            ...
            components=[
                ActionRow(
                    dUse.anyButton(
                        pKey + "rem_selTimeSub_back", "ย้อนกลับไปที่เมนูวิชา", "↩"
                    ),
                ),
                Select(
            ...
            m = await thisChannel.send(
                ...
                components=[
                    ...
                    Select(
                        ...
                        options=[
                            SelectOption(label="1 วัน", value="1"),
                            SelectOption(label="2 วัน", value="2"),
                            SelectOption(label="3 วัน", value="3"),
                            SelectOption(label="4 วัน", value="4"),
                            SelectOption(label="5 วัน", value="5"),
                            SelectOption(label="6 วัน", value="6"),
                            SelectOption(label="1 สัปดาห์", value="7"),
                            SelectOption(label="2 สัปดาห์", value="14"),
                            SelectOption(label="3 สัปดาห์", value="21"),
                            SelectOption(label="1 เดือน", value="30"),
                            SelectOption(label="2 เดือน", value="60"),
                            SelectOption(label="3 เดือน", value="90"),
                        ],
